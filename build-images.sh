...
# Reuse existing nodebuilder-netbox container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-netbox; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-netbox -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:16-alpine docker.io/netboxcommunity/netbox:v4.0-2.9.1 docker.io/redis:7-alpine" \
    "${container}"
# Commit the image
